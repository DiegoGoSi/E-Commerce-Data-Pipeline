...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = pd.merge(pd.merge(items,orders,on='order_id'),products, on='product_id')
    ...
    aggregations = delivered.groupby('order_id').agg({'freight_value':'sum','product_weight_g':'sum'})
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    mask=orders["order_purchase_timestamp"].dt.year==2017
    filtered_dates = orders.loc[mask]
    ...
    order_purchase_ammount_per_date = filtered_dates.groupby(filtered_dates['order_purchase_timestamp'].dt.date)['order_id'].count()
    ...
    result_df = pd.DataFrame({'order_count': order_purchase_ammount_per_date.values,'date': order_purchase_ammount_per_date.index})
    holidays['date']=pd.to_datetime(holidays['date']).dt.date
    result_df['holiday']=result_df['date'].isin(holidays['date']).values
